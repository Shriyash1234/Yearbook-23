...
function Profile(props){
    ...
    const { mail } = location.state !=null ?(location.state):''
            ...
            <div className='profile'>
                <div className='rectangle'>
                    <div className='profile-name-bio'>
                        <p className='profile-name'>{findName(props.name||mail||email)}</p>
                        {
                            responses.map(response =>{
                                return(
                                    ...
                                    {response.Your_Email_ID===(props.name||mail||email) && response.Your_Email_ID !=null ?<p className='bio'>{response.Profile_Biography}</p>:null} 
                                    </div>
                            )})
                        }
                    </div>
                    <div className='img-box'>
                        ...
                        responses.map(response =>{
                            return(
                                <div>
                                {response.Your_Email_ID===(props.name||mail||email) && response.Your_Email_ID !=null ?<img className='profile-photo' src={require('../Assests/pictures/Profile_images/'+funct(response.Profile_Pic))}/>:null}
                                </div>
                        )})
                        }
                    </div>
                ...
                <h2 className='Message-heading'>Messgaes</h2>
                    <div >
                    {
                        responses.map(response =>{
                            return(
                                <div>
                                {response.FEmail===(props.name||mail||email) ?<p className='Message'>{response.Message}?<Link to='/Yearbook-23/students' state={{mail:response.Email}}><em>-{findName(response.Email)}</em></Link></p>:''}
                                </div>
                        )})
                    }
                    </div>
                ...
                <h2 className='Message-heading' >Photo Gallery</h2>
                {
                    responses.map(response =>{
                            return(
                                <div>
                                {response.FEmail===(props.name||mail||email) && response.Photo != null ?<img className='side-photo' src={response.Photo} />:''}
                                </div>
                        )})
                }
